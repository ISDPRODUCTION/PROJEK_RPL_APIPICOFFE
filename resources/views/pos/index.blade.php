...
<div class="flex h-full relative">
    ...
    <div class="flex-1 p-4 md:p-6 overflow-y-auto pb-24 md:pb-6">
        ...
        {{-- Category Tabs (Dinamis dari DB) --}}
        <div class="flex gap-4 md:gap-6 border-b border-stone-200 mb-6 overflow-x-auto scrollbar-hide" id="pos-category-tabs">
            <a href="{{ route('pos.index', ['category' => 'all']) }}"
               class="pb-3 text-sm font-semibold tracking-wider transition-colors relative whitespace-nowrap flex-shrink-0
                       {{ $category === 'all' ? 'text-[#1C1917] after:absolute after:bottom-0 after:left-0 after:right-0 after:h-0.5 after:bg-primary' : 'text-[#78716C] hover:text-[#1C1917]' }}">
                ALL
            </a>
            <div id="pos-dynamic-tabs" class="flex gap-4 md:gap-6"></div>
        </div>
...
<div id="order-panel"
    class="hidden fixed bg-white shadow-xl z-30 select-none flex flex-col
    ...
    {{-- Items List: scroll aktif setelah 3 item (tiap item ±68px → 3×68=204px) --}}
    <div id="order-items-list"
         class="px-4 py-2 space-y-3 overflow-y-auto flex-1"
         style="max-height: 204px; min-height: 0;"></div>

    {{-- Footer: selalu terlihat di bawah --}}
    <div class="px-4 pt-3 pb-6 md:pb-4 border-t border-stone-100 flex-shrink-0">
...
<script>
// ── Load Kategori Dinamis di POS ──────────────────────────────────────────────
(function() {
    var currentCategory = '{{ $category }}';
    fetch('/categories/api?page=1&per_page=50')
        .then(function(r){ return r.json(); })
        .then(function(data) {
            var container = document.getElementById('pos-dynamic-tabs');
            if (!container) return;
            var html = '';
            var baseClass = 'pb-3 text-sm font-semibold tracking-wider transition-colors relative whitespace-nowrap flex-shrink-0';
            var activeClass = 'text-[#1C1917] after:absolute after:bottom-0 after:left-0 after:right-0 after:h-0.5 after:bg-primary';
            var inactiveClass = 'text-[#78716C] hover:text-[#1C1917]';
            (data.data || []).forEach(function(cat) {
                var isActive = currentCategory === cat.slug;
                html += '<a href="/?category=' + cat.slug + '" class="' + baseClass + ' ' + (isActive ? activeClass : inactiveClass) + '">' + cat.name.toUpperCase() + '</a>';
            });
            container.innerHTML = html;
        })
        .catch(function(e){ console.warn('Gagal load kategori POS', e); });
})();
</script>
<script>
